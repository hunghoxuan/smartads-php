...
namespace common\components\vcard;
...
class VCard
{
    ...
    public function getQRCodeURL($width = 500, $height = 500) {
        $content = $this->getOutput();
        return !empty($content) ? "http://chart.apis.google.com/chart?chs={$width}x{$height}&cht=qr&chld=H&chl=" . urlencode($content) : '';
    }

    public function saveFile($file = '', $folder = 'vcard') {
        if (empty($file)) {
            $file = (is_dir($folder) ? $folder : FFile::getFullPath($folder)) . '/' . $this->getFilename() . '.' . $this->getFileExtension();
        } else if (!is_file($file)) {
            $file = FFile::getFullPath($folder) . '/' . $file;
        }

        $file = FHtml::getFullFileName($file);
        FFile::write($file, $this->getOutput());
        return $file;
    }
