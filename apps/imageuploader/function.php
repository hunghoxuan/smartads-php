...
function loadImages() {
    ...
    if(file_exists($useruploadpath)){
        ...
        $subDirs = array_merge([$dir], $subDirs);

        if (is_array($subDirs)) {
            foreach ($subDirs as $subDir) {
                $files = glob("$subDir/*.{jpg,jpe,jpeg,png,gif,ico}", GLOB_BRACE);
                usort($files, create_function('$a,$b', 'return filemtime($a) - filemtime($b);'));
                if (count($files) == 0)
                    continue;
                ?>
                <div style="clear: both; font-weight: bold; font-size: 120%; border: 1px solid lightgrey; background-color: lightgrey; padding: 10px; margin-bottom: 10px; margin-top: 20px"><?= trim(str_replace($dir, '', $subDir), "/") ?></div>
                <div>
                <?php
                for($i=count($files)-1; $i >= 0; $i--):
                    $image = $files[$i];
                    $image_pathinfo = pathinfo($image);
                    //var_dump($image_pathinfo);
                    $image_extension = $image_pathinfo['extension'];
                    $image_filename = $image_pathinfo['filename'];
                    $image_basename = $image_pathinfo['basename'];
                    $image_dir = trim(str_replace($dir, '', $image_pathinfo['dirname']), "/");
                    if (strpos($image_dir, '/') !== false)
                        $image_dir = '';
                    ...
                    $site = $useruploadroot;
                    $image_url = "$site/$useruploadfolder/$image_dir/$image_basename";
