...
include_once(BASE_PATH . '/../../php/main.php');
$application_id = application_id();

$protocol = stripos($_SERVER['SERVER_PROTOCOL'],'https') === true ? 'https' : 'http';
$useruploadroot = "$protocol://$_SERVER[HTTP_HOST]";

$browserfolder = pathinfo("$_SERVER[REQUEST_URI]");
$browserfolder = ltrim($browserfolder["dirname"], '/');
$usersiteroot = substr($_SERVER["SCRIPT_FILENAME"], 0, (stripos($_SERVER["SCRIPT_FILENAME"], "apps/imageuploader/")));

$useruploadfolder = "applications/$application_id/upload";

$useruploadpath = $usersiteroot . "$useruploadfolder/";

$url = $_SERVER[HTTP_REFERER];
$pos = strpos($url, 'backend/web/index.php');
if ($pos === false) {
    $pos = strpos($url, 'admin/');
}
if ($pos === false) {
    $pos = strpos($url, 'apps/imageuploader/');
}
//echo $_SERVER[HTTP_REFERER]; echo $pos;
$useruploadroot = $pos > 0 ? substr($url, 0, $pos) : "$protocol://$_SERVER[HTTP_HOST]";
