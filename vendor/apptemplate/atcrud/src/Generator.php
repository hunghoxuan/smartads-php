...
abstract class Generator extends Model
{
     ...
     * @return string
     */
    public function generateString($string = '', $placeholders = [], $languageNS = '', $multiLang = null)
    {
        if (empty($languageNS))
            $languageNS = $this->messageCategory;

        if (empty($multiLang) || $multiLang == null)
            $multiLang = $this->enableI18N;
        ...
        if ($multiLang) {
            ...
            $str = "FHtml::t('" . $languageNS . "', '" . $string . "'" . $ph . ")";
        } else {
