    ...
    public function actionCreate($type = false)
    {
        ...
        if ($request->isAjax) {
            ...
            if ($model->load($request->post())) {
                ...
                /* @var $model AppUser */
                $model->id = null;
                $model->email = $model->username . "@server3.hotelwifi.in";
                $model->is_active = FConstant::STATE_ACTIVE;
                $model->status = FConstant::LABEL_NORMAL;
                $model->created_date = $today;
                $model->balance = 0;
                $model->rate = 0;
                $model->rate_count = 0;
                $model->role = AppUserAPI::ROLE_USER;
                $model->type = AppUserAPI::TYPE_USER;
                $model->status = AppUserAPI::STATUS_NORMAL;
                $reset_token = md5($now);
