...
if (empty($models)) {
    ...
    for ($j = 0; $j < $i; $j++) {
        $s = $j == 0 ? 1 : 0;
        $models[] = ['receptno' => rand(0, 10), 'patientname' => 'BEN NHAN SO ' . rand(0, 100), 'status_desc' => ($s == 0 ? 'DOI KHAM' : 'DANG KHAM'), 'status' => ($s == 0 ? '0' : 'C')];
    }
    $models[] = ['receptno' => '...', 'patientname' => '...', 'status_desc' => '...', 'status' => '...'];
