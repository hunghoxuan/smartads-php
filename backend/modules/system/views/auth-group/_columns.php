...
return [
    ...
    [ //name: id, dbType: int(11), phpType: integer, size: 11, allowNull:  
        ...
        'value' => function ($model) {
            return '<b>' . FHtml::showContent($model->id, FHtml::SHOW_NUMBER, 'auth_group', 'id', 'int(11)', 'auth-group') . '</b>';
        },
        'hAlign' => 'center',
        'vAlign' => 'middle',
        'width' => '50px',
    ...
    [ //name: name, dbType: varchar(255), phpType: string, size: 255, allowNull:
        ...
        'value' => function ($model) {
            return implode(', ', $model->getRights()) . FHtml::showItemsArray($model->getRolesModels(), '{name}', ['name'], '; ');
        },

    ],
    //    [ //name: name, dbType: varchar(255), phpType: string, size: 255, allowNull:
    //        'class' => FHtml::COLUMN_VIEW,
    //        'label' => FHtml::t('common', 'Users'),
    //        'format'=>'html',
    //        //'attribute'=>'groups',
    //        'hAlign'=>'left',
    //        'vAlign'=>'middle',
    //        'contentOptions'=>['class'=>'col-md-4 nowrap'],
    //        'value' => function($model) {  return FHtml::showItemsArray(\common\models\User::findAll(['id' => $model->getUsers()]), '{name}', ['name'], '; ') ; },
    //
    //    ],
    [ //name: is_active, dbType: tinyint(1), phpType: integer, size: 1, allowNull:  
        ...
        'value' => function ($model) {
            return FHtml::showContent($model->is_active, FHtml::SHOW_BOOLEAN, 'auth_group', 'is_active', 'tinyint(1)', 'auth-group');
        },
        'hAlign' => 'center',
        'vAlign' => 'middle',
        'width' => '20px',
