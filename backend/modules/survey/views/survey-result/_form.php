                                ...
                                <div class="tab-pane active row" id="tab_1_1">
                                    <div class="col-md-12">
                                        <?= FFormTable::widget(['model' => $model, 'form' => $form, 'columns' => 1, 'attributes' => [
                                            'survey_id' => ['value' => $form->fieldNoLabel($model, 'survey_id')->select(FHtml::getComboArray('survey_result', 'survey_result', 'survey_id', true, 'id', 'name'))],
                                            'question_id' => ['value' => $form->fieldNoLabel($model, 'question_id')->select(FHtml::getComboArray('survey_result', 'survey_result', 'question_id', true, 'id', 'name'))],
                                            'customer_id' => ['value' => $form->fieldNoLabel($model, 'customer_id')->select(FHtml::getComboArray('survey_result', 'survey_result', 'customer_id', true, 'id', 'name'))],
                                            'customer_info' => ['value' => $form->fieldNoLabel($model, 'customer_info')->numeric()],
                                            'transaction_id' => ['value' => $form->fieldNoLabel($model, 'transaction_id')->selectMany(FHtml::getComboArray('survey_result', 'survey_result', 'transaction_id', true, 'id', 'name'))],
                                            'comment' => ['value' => $form->fieldNoLabel($model, 'comment')->textarea(['rows' => 3])],
                                            'answer' => ['value' => $form->fieldNoLabel($model, 'answer')->textInput()],
                                            'branch_id' => ['value' => $form->fieldNoLabel($model, 'branch_id')->selectMany(FHtml::getComboArray('survey_result', 'survey_result', 'branch_id', true, 'id', 'name'))],
                                            'employee_id' => ['value' => $form->fieldNoLabel($model, 'employee_id')->selectMany(FHtml::getComboArray('survey_result', 'survey_result', 'employee_id', true, 'id', 'name'))],
                                            'ime' => ['value' => $form->fieldNoLabel($model, 'ime')->textInput()],
                                        ]]); ?>
                                        <?php /*
