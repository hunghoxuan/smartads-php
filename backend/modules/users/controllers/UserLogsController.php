...
use backend\modules\users\models\UserLogsSearch;
...
/**
 * UserLogsController implements the CRUD actions for UserLogs model.
 */
class UserLogsController extends AdminController
{
    protected $moduleName = 'UserLogs';
    protected $moduleTitle = 'User Logs';
    protected $moduleKey = 'user_logs';
    protected $object_type = 'user_logs';
    ...
    public function actionIndex()
    {
        ...
        if (Yii::$app->request->post('hasEditable')) {
            ...
            $model = UserLogs::findOne($Id);

            $out = Json::encode(['output' => '', 'message' => '']);

            $post = [];
            $posted = current($_POST['UserLogs']);
            $post['UserLogs'] = $posted;
    ...
    /**
     * Delete an existing UserLogs model.
     * For ajax request will return json object
    ...
    /**
     * Delete multiple existing UserLogs model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param string $id
     * @return mixed
