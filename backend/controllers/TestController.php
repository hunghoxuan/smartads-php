...
class TestController extends AdminController
{
	...
	public function actionIndex() {
	    $view = FHtml::getRequestParam('view', 'index.php');
	...
	public function actionHtmlToWord() {
		...
		if ($request->post('btnSubmit') == 'word') {
			...
			if (!isset($file)) {
				$template = FHtml::getRootFolder() . '/backend/views/test/template.docx';
			}
			else {
	...
	public function actionHtmlToEXCEL() {
		...
		if ($request->post('btnSubmit') == 'excel') {
			$file = UploadedFile::getInstanceByName('txtFile');
			if (!isset($file)) {
				$template = FHtml::getRootFolder() . '/backend/views/test/template.xlsx';
			}
			else {
				$template = $file->tempName;
			}
